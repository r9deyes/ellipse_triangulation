...
#      print(dir(event))
  ...
  def enter_arg(s,event):
    ...
    if (s.args['A']*s.args['C']-s.args['B']*s.args['B']<=0):
      ...
      s.canvas.itemconfigure(s.cnvWindEntr,state=tk.HIDDEN)
      s.draw()
  def create_line(s,x1,y1,x2,y2):
    s.canvas.create_line(x1+s.mxSize[0]/2,y1+s.mxSize[1]/2,x2+s.mxSize[0]/2,y2+s.mxSize[1]/2)
  def create_rectangle(s,x1,y1,x2,y2,w=0,h=0):
    s.canvas.create_rectangle(x1+s.mxSize[0]/2+w,y1+s.mxSize[1]/2+h,x1+s.mxSize[0]/2-w,y1+s.mxSize[1]/2-h)
  def step1(s):
  ...
  def step4(s,K): ##K:int
    ...
    for i in range(K-1,-1,-1):
      ...
      s.trnglt(int(s.aLi[K-i-1]/h),int(s.aLi[K-i]/h),h,i)
  ...
  def trn(s,N,h,i):
    for j in range(N):
      ...
      s.create_rectangle(x1,y1,x1,y1,1,1)
...
#      y1=(s.b-h*i-h)*sin(j*2*pi/N1+(i%2)*0.523259878)
    ...
    x1=(s.a-h*i-h)*cos(0*2*pi/N1+(i%2)*0.523259878)
    y1=(s.b-h*i-h)*sin(0*2*pi/N1+(i%2)*0.523259878)
    x3=(s.a-h*i)*cos(0*2*pi/N2+((i+1)%2)*0.523259878)
    y3=(s.b-h*i)*sin(0*2*pi/N2+((i+1)%2)*0.523259878)
    i1+=1
    i2+=1
    while (i1<N1 or i2<N2):
      if (i1/i2>N1/N2):
        if(i2<=N2):
          x2=(s.a-h*i)*cos(i2*2*pi/N2  +((i+1)%2)*0.523259878)
          y2=(s.b-h*i)*sin(i2*2*pi/N2  +((i+1)%2)*0.523259878)
          i2+=1
      else:
        if (i1<=N1):  
          x1=(s.a-h*i-h)*cos(i1*2*pi/N1  +(i%2)*0.523259878)
          y1=(s.b-h*i-h)*sin(i1*2*pi/N1  +(i%2)*0.523259878)
          i1+=1
          
      s.create_line(x1,y1,x2,y2)
      s.create_line(x3,y3,x2,y2)
      s.create_line(x1,y1,x3,y3)
      sleep(100)
      if (i1/i2>N1/N2):
        x3=x2
        y3=y2
...
#
    ...
    try:
      ...
      s.x3=(s.a-h*i)*cos(s.i2*2*pi/N2  +((i+1)%2)*0.523259878)
      s.y3=(s.b-h*i)*sin(s.i2*2*pi/N2  +((i+1)%2)*0.523259878)
      s.x1=(s.a-h*i-h)*cos(s.i1*2*pi/N1  +(i%2)*0.523259878)
      s.y1=(s.b-h*i-h)*sin(s.i1*2*pi/N1  +(i%2)*0.523259878)
      s.i2+=1
      s.i1+=1
    if(1):#while (i1<N1 or i2<N2):
      if (s.i1/s.i2>N1/N2):
        if(s.i2<=N2):
          s.x2=(s.a-h*i)*cos(s.i2*2*pi/N2  +((i+1)%2)*0.523259878)
          s.y2=(s.b-h*i)*sin(s.i2*2*pi/N2  +((i+1)%2)*0.523259878)
          s.i2+=1
      else:
        if (s.i1<=N1):  
          s.x1=(s.a-h*i-h)*cos(s.i1*2*pi/N1  +(i%2)*0.523259878)
          s.y1=(s.b-h*i-h)*sin(s.i1*2*pi/N1  +(i%2)*0.523259878)
          s.i1+=1
      s.canvas.create_line(s.x1+s.mxSize[0]/2,s.y1+s.mxSize[1]/2,s.x2+s.mxSize[0]/2,s.y2+s.mxSize[1]/2)
#      s.canvas.create_rectangle(x1+s.mxSize[0]/2,y1+s.mxSize[1]/2,x1+s.mxSize[0]/2+2,y1+s.mxSize[1]/2+2)
      s.canvas.create_line(s.x3+s.mxSize[0]/2,s.y3+s.mxSize[1]/2,s.x2+s.mxSize[0]/2,s.y2+s.mxSize[1]/2)
#      s.canvas.create_rectangle(x2+s.mxSize[0]/2,y2+s.mxSize[1]/2,x2+s.mxSize[0]/2+2,y2+s.mxSize[1]/2+2)
      s.canvas.create_line(s.x1+s.mxSize[0]/2,s.y1+s.mxSize[1]/2,s.x3+s.mxSize[0]/2,s.y3+s.mxSize[1]/2)
